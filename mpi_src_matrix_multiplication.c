...
int main(int argc, char** argv)
{
	...
	for (int kk = 0; kk < PP; kk++) // KOLENA ITERACJA PRZETWARZANIA (OBLICZENIA I KOMUNIKACJA) 
	{
		...
		MPI_Sendrecv(psa, BlockSize*BlockSize, MPI_FLOAT, left_neighbor, tag, pra, BlockSize*BlockSize, MPI_FLOAT, right_neighbor, tag, MPI_COMM_WORLD, &statRecv[0]);
		MPI_Sendrecv(psb, BlockSize*BlockSize, MPI_FLOAT, up_neighbor, tag, prb, BlockSize*BlockSize, MPI_FLOAT, down_neighbor, tag, MPI_COMM_WORLD, &statRecv[1]);
