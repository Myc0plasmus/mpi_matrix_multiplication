...
int main(int argc, char** argv)
{
	...
	if (my_rank == 0)
	{
		...
		for(int process_rank =0;process_rank<P;process_rank++){
			...
			int px = process_rank % PP;
			int py = process_rank / PP;

			int starts[2] = {py * BlockSize, px * BlockSize};

			// MPI_Type_create_subarray(2,arrayDims,subarrayDims,starts,MPI_ORDER_C, MPI_FLOAT, &block);
			// MPI_Type_commit(&block);
			
			MPI_Recv(&Cglob[starts[1]][starts[0]] ,BlockSize * BlockSize,MPI_FLOAT,process_rank,tag,MPI_COMM_WORLD, &status);
